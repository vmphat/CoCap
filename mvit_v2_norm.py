...
def chunk_frames(inputs, chunk_size, stack_size, step_size):
    ...
    for i in range(0, T, step_size):
        ...
        assert chunk.shape[1] == stack_size
...
video_paths = sorted(Path(f"./{CORPUS.lower()}/videos_{RESIZE}_h264_keyint_{KEYINT}/").glob(f"*.{EXTENSION}"))
...
# !Only remove `head` to get feature vectors
model.head = torch.nn.Identity()
model.eval()

# --- extract features for all videos ---
vid_to_gop_count: Dict[str, int] = {}
mot_emb_hdf5 = h5py.File(f"./{CORPUS.upper()}_newNormMViTv2.hdf5", "w")
