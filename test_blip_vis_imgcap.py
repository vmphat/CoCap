...
# video_paths = Path(f"./videos/").glob(f"*.{EXTENSION}")
video_paths: List[str] = list(Path(f"./videos/").glob(f"*.{EXTENSION}"))
...
blip_model = BlipForConditionalGeneration.from_pretrained("Salesforce/blip-image-captioning-base", cache_dir=CACHE_DIR).to(device)
embed_model = SentenceTransformer("all-MiniLM-L6-v1", device=device, cache_folder=CACHE_DIR)
